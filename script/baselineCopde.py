...
def load_drivecycle_mat_files(base_folder: str):
    ...
    We assume each .mat has exactly one “user variable” that is an N×2 array:
      column 0 = time (s), column 1 = speed (kph).
    """
...
def choose_cycle_key(all_cycles):
    """
    Print all available cycle keys and ask the user to pick one.
    Returns the chosen key as a string.
    """
    keys = list(all_cycles.keys())
    print("Available drive cycles:")
    for i, k in enumerate(keys, start=1):
        print(f"  [{i}] {k}")

    while True:
        choice = input("Enter the number or name of the cycle you want to use: ").strip()
        # First check if the user typed a (valid) index:
        if choice.isdigit():
            idx = int(choice) - 1
            if 0 <= idx < len(keys):
                return keys[idx]
            else:
                print(f"  → Invalid index '{choice}'. Please pick a number between 1 and {len(keys)}.")
                continue

        # Otherwise, check if the user typed a valid key name:
        if choice in keys:
            return choice

        print(f"  → '{choice}' is not a valid index or cycle name. Try again.\n")
...
if __name__ == "__main__":
    ...
    # Prompt the user:
    chosen_key = choose_cycle_key(all_cycles)
    cycle_data = all_cycles[chosen_key]
    print(f"\n[Main] Using reference cycle '{chosen_key}'")
    ...
    max_delta = 20.0             # maximum % change per 0.01 s tick
    ...
    try:
        while True:
            ...
            if u >= 0.0:
                ...
                # set_duty(0, 0.0)   # accelerator off             
                # set_duty(4, 0.0)   # channel 4 = brake off               
                set_duty(4, -u)    # channel 4 = brake = |u|
        ...
        # zero out any remaining channels if still initialized
        if pca_initialized:
            for ch in range(16):
                try:
