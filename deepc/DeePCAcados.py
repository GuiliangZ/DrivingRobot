...
class deepctools():
            ...
            ------Initialize the system parameters and DeePC config------                                                       | ----- Drive Robot --------
                 ...
                 y_dim: [int]             |  the dimension of controlled outputs                                                |           1
                  ...
                  Tini: [int]             |  the initialization length of the online loop                                       |   Hyperparameter to tune
                    ...
                    ud: [array]           |  history data collected offline to construct Hankel matrix; size (T, u_dim)         |   (data length, 1)
                    yd: [array]           |  history data collected offline to construct Hankel matrix; size (T, u_dim)         |   (data length, 1)
             ...
             sp_change: [bool]            |  whether the set-point of u and y change during control,                            |   Always true as I have constantly changing reference
                                          |      if True, uref, yref will be parameters of the optimization problem  *In my problem, it will change yref, don't know uref
                                          |      if False, can just give ys, us
                    us: [array]           |  set-point of u;  size (1, u_dim), if sp_change=False, can just define us, ys       |   not using as sp_change = True
                    ys: [array]           |  set-point of y;  size (1, y_dim), if sp_change=False, can just define us, ys       |   not using as sp_change = True
    ...
    def _checkvar(self):
        """
            ---------Check the variables if consist with DeePC config-------------------------------------------------------------------------------
                  Variables    |             Shape                            |          Drive Robot(T=753306, Tini=50, Np=50, L=100, u_dim=1)
            -------------------|-------------------------------------------------------------------------------------------------------------------
            ud, yd             |  (T, dim)                                    |   ud, yd-same length = (Total data length(753405), 1)                         
            Hud, Hyd           |  (dim*L, T-L+1), L = Tini + Np, if svd=False |   Hud, Hyd-same length = (1*100, 753306)
                               |  (dim*L, rank([Hud, Hyd]), if svd=True       |   Not use svd  
            Up, Yp             |  (dim*Tini, T-L+1)                           |   (1*50, 753306)  
            Uf, Yf             |  (dim*Np, T-L+1)                             |   (1*50, 753306)
            uref, yref         |  (dim*Np, 1)                                 |   (1*50, 1)
            Q, R               |  (dim*Np, dim*Np)                            |   (1*50, 1*50)
            lambda_g           |  (T-L+1, T-L+1)                              |   (753306, 753306)
            lambda_y           |  (dim*Tini, dim*Tini)                        |   (1*50, 1*50)
            ------------------------------------------------------------------
            Persistently Excitation condition:
                1. g_dim >= u_dim * (Tini + Np)                               |   (g_dim=T-Tini-Np+1=753306) > 1 * (50+50)  In my system definitely the case, I have very large hankel matrix
                2. the Hankel matrix of ud should be full row rank            |   hankel matrix at least have rank of: 1 * 100
    ...
    @timer
    def init_DeePCAcadosSolver(self, opts={}):
        """
        ...
        """
        # 1) Dimensions
        self.g_dim = self.Uf.shape[1]
        self.Tini = self.Up.shape[0]
        self.Np = self.horizon  # future steps count
